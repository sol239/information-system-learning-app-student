<template>
    <UModal v-model:open="modalOpen" fullscreen :dismissible="false" :title="t('task_modal_title')">
        <UButton icon="i-lucide-list-todo" color="neutral" variant="soft" :label="selectedTask?.title || t('task_modal_title')" />

        <template #body>
            <div class="flex flex-col gap-4">
                <div class="flex items-center justify-between gap-4">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        {{ t('task_modal_description') }}
                    </p>
...
const importFromJson = async () => {
    ...
    system.defaultTasks = system.tasks.map(t => Task.fromJSON(JSON.parse(JSON.stringify(t))))
